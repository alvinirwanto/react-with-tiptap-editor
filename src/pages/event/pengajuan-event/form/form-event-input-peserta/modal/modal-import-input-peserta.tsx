import { addImportPesertaMutationFn } from '@/api/admin/api-event';
...
interface ModalImportInputPesertaProps {
    isOpen: boolean;
    onClose: () => void;
    addListInputPeserta: any;
}
export default function ModalImportInputPeserta({
    isOpen,
    onClose,
    addListInputPeserta
}: Readonly<ModalImportInputPesertaProps>) {
    ...
    const { mutate: mutateUploadFile, isPending } = useMutation({
        mutationFn: addImportPesertaMutationFn,
    });
    ...
    const onSubmit = async (data: any) => {
        ...
        mutateUploadFile(
            ...
            {
                onSuccess: (response) => {
                    addListInputPeserta(response.data.data)
                    form.reset()
                    onClose()
                },
                onError: (error) => {
        ...
        <>
            ...
            >
                <div className='flex flex-col gap-4'>
                    <h4 className='text-xl font-semibold'>Import Peserta</h4>
                    <Form {...form}>
